...
function ResponseDisplay({ response }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            {(autoExecuteResult || autoExecuteError) && (
              <span className="flex items-center space-x-1 text-sm text-blue-600 px-3 py-1 bg-blue-50 rounded-md">
      ...
      {/* 查询结果 */}
      {(autoExecuteError || executeError) && (
        <div className="space-y-2">
          <div className="flex items-center space-x-2 text-red-600">
            <AlertCircle className="w-4 h-4" />
            <span className="font-medium">
              {autoExecuteError ? '自动执行失败' : '执行失败'}
            </span>
          </div>
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-red-700">{autoExecuteError || executeError}</p>
          </div>
        </div>
      )}

      {(autoExecuteResult || executeResult) && (() => {
        const currentResult = autoExecuteResult || executeResult
        return (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2 text-green-600">
                ...
                <span className="text-sm text-gray-500">
                  ({currentResult.row_count} 行, {currentResult.execution_time?.toFixed(3)}秒)
                </span>
              </div>
              {currentResult.data && currentResult.data.length > 0 && (
                <button
                  onClick={() => copyToClipboard(JSON.stringify(currentResult.data, null, 2), 'result')}
                  className="flex items-center space-x-1 text-sm text-gray-600 hover:text-gray-800 transition-colors"
                >
                  {copiedStates.result ? (
                    <>
